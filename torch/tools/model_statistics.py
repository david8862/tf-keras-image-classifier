...
def main():
    ...
    parser.add_argument('--model_input_shape', type=str, required=True, help='model input image shape as <channel>x<height>x<width>')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = torch.load(args.model_path, map_location=device)

    channel, height, width = args.model_input_shape.split('x')
    input_tensor = torch.randn(1, int(channel), int(height), int(width)).to(device)
