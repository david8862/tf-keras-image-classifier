...
def classifier_http_client(server_addr, server_port, model_name, image_files, class_names, output_path):
    ...
    assert len(input_config) == 1, 'invalid input number.'
    assert len(output_config) == 1, 'invalid input number.'

    input_name = input_config[0]['name']
    input_type = input_config[0]['data_type']
    output_name = output_config[0]['name']
    output_type = output_config[0]['data_type']

    # check input & output config
    assert input_type == 'TYPE_FP32', 'invalid input type.'
    assert output_type == 'TYPE_FP32', 'invalid output type.'

    assert len(input_config[0]['dims']) == 4, 'invalid input dims.' # NCHW
    batch, channel, height, width = input_config[0]['dims']
    model_input_shape = (height, width)

    assert len(output_config[0]['dims']) == 2, 'invalid output dims.' # (1, num_classes)
    num_classes = output_config[0]['dims'][1]
    if class_names:
...
def classifier_grpc_client(server_addr, server_port, model_name, image_files, class_names, output_path):
    ...
    # get input & output config
    model_config = triton_client.get_model_config(model_name)
    input_config = list(model_config.config.input)
    output_config = list(model_config.config.output)

    assert len(input_config) == 1, 'invalid input number.'
    assert len(output_config) == 1, 'invalid input number.'

    input_name = input_config[0].name
    input_type = grpc_data_type_str(input_config[0].data_type)
    output_name = output_config[0].name
    output_type = grpc_data_type_str(output_config[0].data_type)
...
def main():
    parser = argparse.ArgumentParser(description='classifier http client for triton inference server')
